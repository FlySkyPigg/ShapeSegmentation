...
from models.pointnet_part_seg import get_model, get_loss
import torch
...
parser.add_argument('--experiment_name', type=str, default='output_seg_ssg_pointnet_with_jitter')

# data augmentation argument. keep align with CloserLook3D.
scale_low = 0.7
scale_high = 1.3
noise_std = 0.001
noise_clip = 0.05
...
if __name__ == "__main__":
    ...
    train_transforms = transforms.Compose([
        ...
        d_utils.PointcloudScale(scale_low=scale_low, scale_high=scale_high),
        d_utils.PointcloudJitter(std=noise_std, clip=noise_clip),
    ])
    ...
    net = get_model(part_num=num_sem_class, normal_channel=False).cuda()
    loss_fn = get_loss()
    ...
    for epoch in tqdm(range(args.max_epoch)):
        ...
        for pcs, masks, point_labels, labels in train_dataloader:
            ...
            pred_sem, trans_feat = net(pcs, onehot_labels)
            ...
            loss = loss_fn(pred_sem, point_labels, trans_feat)
        ...
        if epoch % args.epochs_per_eval == 0:
            ...
            for pcs, masks, point_labels, labels in val_dataloader:
                ...
                pred_sem = torch.reshape(pred_sem, shape=(-1, num_sem_class)).contiguous()
                point_labels = torch.reshape(point_labels, shape=(-1,)).contiguous()
                loss = F.nll_loss(pred_sem, point_labels)
