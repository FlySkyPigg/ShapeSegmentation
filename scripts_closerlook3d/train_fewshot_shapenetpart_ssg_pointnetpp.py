from datasets.ShapeNetPart import FewShotShapeNetPart
import argparse
from torch.utils.data import DataLoader
from models.pointnet2_part_seg_ssg_fewshot import get_model, get_loss
import torch
...
parser.add_argument('--experiment_name', type=str, default='output_fewshot_seg_ssg_pointnetpp')
...
if __name__ == "__main__":
    train_dataset = FewShotShapeNetPart(num_points=2048, data_root=args.data_root, category='Table', split='train')
    train_dataloader = DataLoader(dataset=train_dataset, batch_size=args.batch_size, shuffle=False)

    num_sem_class = train_dataset.num_parts
    ...
    for epoch in tqdm(range(args.max_epoch)):
        ...
        net.train()
        epoch_loss = []
        for sx, sy, qx, qy in train_dataloader:
            sx, sy, qx, qy = sx.cuda(), sy.cuda(), qx.cuda(), qy.cuda()
            pred_qy = net(sx, sy, qx, train_dataset.num_parts)

            pred_sem = torch.reshape(pred_qy, shape=(-1, num_sem_class)).contiguous()
            point_labels = torch.reshape(qy, shape=(-1,)).contiguous()
